...
上の Android 画面と CLI 画像は 1.4.2 リリース候補から作成しています。Android アプリと CLI は英語、簡体字中国語、日本語、韓国語に完全対応します。

## ダウンロード

[GitHub Releases](https://github.com/sampple-korea/AuroraPure/releases/latest) から取得してください。

> **1.4.0 から署名鍵が変わりました。** 1.3.0 以前に上書きインストールできません。
> 先に以前のバージョンをアンインストールしてください。保存フォルダにある APK は
> そのまま残り、アプリ内のダウンロード履歴はアンインストールとともに失われます。

| プラットフォーム | ファイル | 要件 |
| --- | --- | --- |
| Android | `AuroraPure-1.4.2.apk` | Android 10 / API 29 以降 |
| Linux・macOS CLI | `aurora-pure-cli-1.4.2.tar` または `.zip` | Java 21 以降 |
| Windows CLI | `aurora-pure-cli-1.4.2.zip` | Java 21 以降、`bin\aurora-pure.bat` を実行 |
...
Google Play は端末機能やグラフィックテクスチャ形式など、さらに別の次元で配信を最適化する場合があります。1.4.2 が明示的に扱うのは ABI、密度、Android バージョン、言語です。
