...
The Android screenshots and CLI capture above are generated from the 1.4.0 release candidate. The interface is fully available in English, Simplified Chinese, Japanese, and Korean.

## Download

Get release files from [GitHub Releases](https://github.com/sampple-korea/AuroraPure/releases/latest).

> **1.4.0 is signed with a new key.** Android will not install it over 1.3.0 or earlier.
> Uninstall the previous version first. Files already saved to your download folder are
> unaffected; the in-app download history is lost with the uninstall.

| Platform | Release file | Requirement |
| --- | --- | --- |
| Android | `AuroraPure-1.4.0.apk` | Android 10 / API 29 or newer |
| Linux and macOS CLI | `aurora-pure-cli-1.4.0.tar` or `.zip` | Java 21 or newer |
| Windows CLI | `aurora-pure-cli-1.4.0.zip` | Java 21 or newer; run `bin\aurora-pure.bat` |
...
Google Play may target other dimensions, such as device features or graphics texture formats. Version 1.4.0 covers the ABI, density, Android-version, and language dimensions it explicitly probes—not every possible Play targeting dimension.
